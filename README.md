...
(TODO) Efficiently computes size of bitstrings size 0..(2^44-1) bits and if you only need a powOf2 size (cbt) it efficiently counts that to 2^32766 bits (which is somewhere between a googol and a googolplex number of bits, per object), and unlimited size but slower using interpreted mode as everything is lambda called on lambda returns lambda. Each such bitstring has a 256 bit (32 byte) lazyevaled global id. For example, the ethereum state space could in theory be modelled as bitstrings of around 2^256 bits each (maybe 2^9 times that?) using a depth 64 tree of 16 branches each (skipping internal nodes), or a 256 depth tree of 2 branches each, or more efficient would be to use a trie-like treemap that skips levels where theres only 1 path. Though it probably would be alot slower than that existing code due to sharding strategies, its just an example.

The data format, designed to cross untrusted borders safely, comes in 96-byte self-proving statements (other than Op.ax which has to be further verified that (axA thing) and (axB thing) dont both exist literally in the 2-way forest nor can they be derived from it (but if you stay in the CLEAN area, not the DIRTY area, and a forest node does not literally contain any ax statements (of 2 params, as it verifies constraint at 2 params, evals normally at 3 params)... then its guaranteed it cant generate such errors nomatter what combos of anything called on anything (all infinity possible lambda functions it can create) and the 32 bytes tells if its CLEAN vs DIRTY and if it contains such ax statements or not, which are 3 ids of 32 bytes each, a parent and its 2 childs, or just 32 bytes if its a literal bitstring. Most random 256 bits are their own id. Any id that starts with the byte 0xf9 is a normal id, which is either a normal call pair or 1, 2, 4, 8, 16, 32, 64, or 128 literal bits. The whole system state is a 2-way forest of 32 byte ids (a marklar105b id). There will be a deterministic function of 64 bytes to 32 bytes which can run on CPU in a few microseconds and run many times in parallel in GPU to compute maybe 100 million to a billion of them per second (just a guess), though the GPU cant compute just any turing complete logic efficiently it will in some cases like matrix multiply, and such a matrix will have a 32 byte global id, as will a matrix multiply algorithm, as will various optimizations of algorithms, each have their own 32 byte id. A 96 byte statement is valid if its last 64 bytes generate the first 32 bytes, and it hooks into another 96 byte statement by matching the second and third ids in it to the first id in those. If you put a bunch of these 96 byte statements in a bag and randomly reorder them, they can be reassembled into useful turing complete calculations and can derive more such 96 byte statements, and can do that in an unlimited number of threads (such as 1 billion computers andOr people each using 8 CPUs and a few thousand GPU cores and it will all fit together like clockwork, or smaller simpler experiments to get started. Ids are lazyevaled and are normally computed by == in a hashtable, generating ids only when used as treemap keys or checking for equality or crossing untrusted borders or in some ways of storing them on harddrive. This code is (incomplete) and in public static long[] parentId(long leftHeader, long leftB, long leftC, long leftD, long rightHeader, long rightB, long rightC, long rightD)" in https://github.com/benrayfield/wikibinator105/blob/main/wikibinator105/impls/marklar105b/ids/MarklarId105b.java and the main logic (interpreted mode, not optimizations of it yet) in https://github.com/benrayfield/wikibinator105/blob/main/wikibinator105/impls/marklar105b/evalers/InterpretedModeUsingJavaStack.java
...
* Header size of a function id is 1-64 bits and is deterministicly generated from the headers of its 2 childs and 1 byte from each of where is their last 1 bit in those 256 bits in case they are literal 256 bits thats its own id which happens when the first byte is not 0xf9 (so most random 256 bits are their own id, a 1/8 of a bit size header). Similarly, every id256 is deterministicly generated from its 2 child id256s, and is lazyEvaled, needing only the header and that "where is their last 1 bit" such as to wrap an int[234221] in a lambda without generating its id256 yet or ever.



2021-2-11 Here are the 1-byte opcodes generated, from their Human readable form, in a loop in
https://github.com/benrayfield/wikibinator105/blob/main/wikibinator105/spec/Op.java
These will be used in a switch statement in
https://github.com/benrayfield/wikibinator105/blob/main/wikibinator105/impls/marklar105b/evalers/InterpretedMode.java
